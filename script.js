...
function initializeSessionData() {
    ...
    if (savedFirstName) {
        ...
        // NEW USER - show welcome message but keep same title
        document.getElementById('welcomeTitle').textContent = 'Welcome to Oasis Health Care!';
        
        // Hide "Not [Name]?" section
        document.getElementById('notYouSection').style.display = 'none';
    }
    
    // Get or initialize visit count (Local Storage) - Still track but don't display
    ...
    // Get last visit from cookie - Still track but don't display
    ...
    // Store session start time in sessionStorage - Still track but don't display
...
function showSuccess(element) {
    ...
    // Check form validity after showing success
    checkFormValidity();
}
...
function clearError(element) {
    ...
    // Check if all required fields are valid after clearing error
    checkFormValidity();
}
...
function checkFormValidity() {
    ...
    // Check all required text/email/date inputs
    ...
    // Check if SSN is filled (9 digits)
    const ssnField = document.getElementById('ssn');
    if (!ssnField || !ssnField.value || ssnField.value.length !== 9) {
        allValid = false;
    }
    ...
    // Check if at least one medical history checkbox is checked
