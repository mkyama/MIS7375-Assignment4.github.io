...
// Set cookie with 48-hour expiration (with localStorage fallback)
...
// Get cookie by name (with localStorage fallback)
...
// Delete/expire cookie (with localStorage removal)
...
function handleRememberMe() {
    ...
    if (!rememberMe.checked) {
        ...
        deleteCookie('oasisUserFirstName');
        clearLocalStorage();
        console.log('🗑️ Remember Me unchecked - cookie and data cleared');
    } else if (firstName) {
        // User re-checked - save firstName to cookie immediately
        setCookie('oasisUserFirstName', firstName, 48);
        console.log('✅ Cookie saved from handleRememberMe:', firstName);
    }
}

// Auto-save firstName to cookie as user types (if Remember Me is checked)
function autoSaveFirstNameToCookie() {
    const rememberMe = document.getElementById('rememberMe');
    const firstName = document.getElementById('firstName').value.trim();
    
    // Only save if Remember Me is checked and firstName has a value
    if (rememberMe && rememberMe.checked && firstName) {
        setCookie('oasisUserFirstName', firstName, 48);
        console.log('💾 Auto-saved first name to cookie:', firstName);
    }
}
...
function setupWelcomeMessage() {
    ...
    if (firstName) {
        ...
        welcomeMsg.style.color = '#4B0082';
        
        // Dynamically create "Not me?" checkbox (like friend's code)
        ...
        checkbox.id = "notYouCheckbox";
        checkbox.style.marginRight = "5px";
        checkbox.style.marginLeft = "5px";
        
        label.appendChild(checkbox);
        label.appendChild(document.createTextNode(` Not ${firstName}? Click here to start as a NEW USER.`));
        newUserCheckbox.appendChild(label);
        
        // Add event listener for "Not me?" checkbox
        checkbox.addEventListener("change", function() {
            if (this.checked) {
                // Clear cookie and localStorage immediately
                deleteCookie('oasisUserFirstName');
                clearLocalStorage();
        ...
        // Pre-fill the first name field with the cookie value
        const firstNameField = document.getElementById('firstName');
        if (firstNameField) {
            firstNameField.value = firstName;
            console.log("✅ First name pre-filled:", firstName);
        }
    } else {
        // New user (no cookie)
        welcomeMsg.textContent = 'Welcome, New User! 🌟';
...
// ============================================================================
// FORM SUBMISSION - SAVE COOKIE ON SUBMIT
// ============================================================================
...
function handleFormSubmission(event) {
    ...
    console.log('   Remember Me checked:', rememberMe ? rememberMe.checked : false);
    console.log('   First Name:', firstName);
    
    // Save cookie if "Remember Me" is checked and we have a first name
    if (rememberMe && rememberMe.checked && firstName) {
        console.log('✅ Saving cookie for:', firstName);
        setCookie('oasisUserFirstName', firstName, 48); // 48 hours
    } else {
        console.log('ℹ️ Cookie not saved (Remember Me unchecked or no first name)');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add form submission handler to save cookie
    const patientForm = document.getElementById('patient-form');
    if (patientForm) {
        patientForm.addEventListener('submit', handleFormSubmission);
        console.log('✅ Form submission handler added');
    }
    
    // Add auto-save for firstName field (saves cookie as user types)
    const firstNameField = document.getElementById('firstName');
    if (firstNameField) {
        // Save when user leaves the field
        firstNameField.addEventListener('blur', autoSaveFirstNameToCookie);
        // Also save as they type (after a short delay)
        let firstNameTimeout;
        firstNameField.addEventListener('input', function() {
            clearTimeout(firstNameTimeout);
            firstNameTimeout = setTimeout(autoSaveFirstNameToCookie, 1000); // Save 1 second after they stop typing
        });
        console.log('✅ First name auto-save enabled');
    }
});
